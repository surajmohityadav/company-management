				...
				<input type="hidden" name="id" value="<?php echo isset($id) ? $id : '' ?>">
					<div class="col-md-6">
						<div class="form-group">
							<label for="" class="control-label">No. of pages in Album</label>
							<input type="text" name="no_of_p" class="form-control form-control-sm" required value="<?php echo isset($no_of_p) ? $no_of_p : '' ?>">
						</div>
						
						<div class="form-group">
							<label for="" class="control-label">Material</label>
							<select name="m_id" id="m_id" class="custom-select custom-select-sm select2">
								<option value=""></option>
							<?php
								$material = $conn->query("SELECT * FROM material order by m_id asc");
								while($row = $material->fetch_assoc()):
							?>
								<option value="<?php echo $row['id'] ?>" <?php echo isset($m_name) && $m_name == $row['id'] ? "selected" : '' ?>><?php echo ucwords($row['name']) ?>
							<?php endwhile; ?>
							</select>
						</div>
